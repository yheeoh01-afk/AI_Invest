...
# -----------------------------
# Streamlit Cloud 로그인 / 사용자별 저장소 / 모바일 CSS
# -----------------------------
import json
import hashlib

WEB_DATA_ROOT = os.environ.get("AI_INVEST_DATA_DIR", "user_data")
...
def get_login_users():
    """
    Streamlit Cloud Secrets 지원 형식:
    1) USERS='{"younghee":"1234", "admin":"admin123"}'
    2) [users]
       younghee="1234"
       admin="admin123"
    """
    ...
    # 처음 배포 테스트용 기본 계정. 실제 사용 시 Streamlit Secrets의 USERS로 바꾸세요.
    if not users:
        users = {"admin": "1234"}
...
def render_mobile_css():
    st.markdown(
        ...
        @media (max-width: 768px) {
            ...
            .stTabs [data-baseweb="tab"] { white-space: nowrap; font-size: 0.86rem; }
            .portfolio-table { font-size: 12px !important; min-width: 1400px !important; }
            .portfolio-table th, .portfolio-table td { padding: 6px 8px !important; }
        }
        </style>
        """,
...
def login_gate():
    ...
    st.caption("사용자별 포트폴리오가 분리 저장됩니다.")
    ...
    if submitted:
        ...
        if user_id in users and str(users[user_id]) == str(password):
    ...
    st.info("Streamlit Cloud에서는 App settings → Secrets에 USERS와 NAVER API 키를 넣어주세요.")
    st.stop()


def configure_user_files():
    global PORTFOLIO_FILE, WATCHLIST_FILE, CSV_FILE

    user_id = _safe_username(st.session_state.get("user_id", "default"))
    user_dir = os.path.join(WEB_DATA_ROOT, user_id)
    os.makedirs(user_dir, exist_ok=True)

    PORTFOLIO_FILE = os.path.join(user_dir, "portfolio.csv")
    WATCHLIST_FILE = os.path.join(user_dir, "watchlist.csv")
...
CURRENT_USER, CURRENT_USER_DIR = configure_user_files()
...
with st.expander("➕ 포트폴리오 추가/수정/삭제", expanded=False):
    ...
    if uploaded_portfolio is not None:
        try:
            uploaded_df = pd.read_csv(uploaded_portfolio, encoding="utf-8-sig")
        except Exception:
            uploaded_portfolio.seek(0)
            uploaded_df = pd.read_csv(uploaded_portfolio, encoding="cp949")
...
with tab2:
    ...
    with right_add:
        ...
        if uploaded_watchlist is not None:
            try:
                uploaded_wdf = pd.read_csv(uploaded_watchlist, encoding="utf-8-sig")
            except Exception:
                uploaded_watchlist.seek(0)
                uploaded_wdf = pd.read_csv(uploaded_watchlist, encoding="cp949")
            save_watchlist(uploaded_wdf)
            st.success("업로드한 관심그룹 CSV를 현재 로그인 사용자 계정에 저장했습니다.")
            st.rerun()
